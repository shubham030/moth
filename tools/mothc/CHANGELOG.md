...
- `package:analyzer` 7 → 14: the AST migration (class bodies, the new
  `Argument`/`NamedArgument` model, default clauses on parameters). All 84
  tests pass unchanged — goldens generated by the real Dart SDK pin that
  the language behavior did not move.
- An `example/` walking the five-command loop.
- `create` now suggests `moth run` as the next step.
