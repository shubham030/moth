...
MCUs. (Google's own MCU attempt, Dartino, was shelved; its repo has been
archived since 2020.)
...
patching the backend's retained node tree — moth_render's today (ADR-008) —
which we prefer anyway (ADR-005).
...
the compiler, and interpreter throughput is not our bottleneck — moth_render does the
per-frame work in C++ (measured: ~4.5ms of a 26.2ms frame is VM rebuild);
bytecode runs event handlers and diffs. Revisit with
...
<a id="adr-005"></a>
## ADR-005: Two trees, not three — the backend is the render layer

**Status update (Aug 2026):** the decision — two trees, no RenderObject
layer — stands. The render layer is `moth_render`, not LVGL (ADR-008);
Elements hold `mr_node_id`s. Read `lv_obj` below as "the backend's retained
node".
...
binding layer was never built, and ThorVG was never adopted — the shipped
rasterizer is moth's own. moth_render is the de facto primary backend;
...
named parameters, getters and setters (enums once the compiler accepts
them). Arduino's naming is used only
...
**Why the flat functions exist:** they are the boundary the VM calls —
minimal and C-shaped because that is what resolves cheaply at load time.
`package:moth` wraps every one of them, and the docs point at the wrappers.
