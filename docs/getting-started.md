...
pushed over /dev/cu.usbmodem2101 in 380ms
```

> The push protocol's verdict reply is verified on the desktop simulator
> and in the test suite; its on-board delivery has one known race under
> heavy console logging that is awaiting hardware confirmation. If a push
> that visibly took reports "no reply", that is it — reset and re-push.
